...
const { agregarEmpleado, crearUsuario, crearCompetencia, crearPregunta, crearEncuesta, crearPreguntaEncuesta, insertarHabilidades } = require("../services/createService");
...
async function crear_pregunta(request, response) {
    const errorMessages = [];
    const {idCompetencia, idHabilidad, pregunta, comportamiento, lenguaje, tipo} = request.body;
    const preguntaData = {
...
async function asignar_habilidades(request, response) {
    ...
    for(const habilidad of habilidades) {
        const result = await insertarHabilidades({id_pefil_puesto: idPuesto, id_habilidad: habilidad.idHabilidad});
    }
}
