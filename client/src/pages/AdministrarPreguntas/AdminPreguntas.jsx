...
const AdminPreguntas = () => {
  ...
  const handleGetHabilidades = () => {
    axios
      .get("http://localhost:4000/competencias/")
      .then((response) => {
  ...
  const handleObtenerEncuesta = () => {
    ...
    axios
      .get(
        `http://localhost:4000/entrevistas-competencia/obtener-por-id-perfil-puesto/${id_perfil_puesto}`
      )
      .then((response) => {
  ...
  return (
    <div className="questions-admin-container">
      ...
      <div className="evaluations-actions">
        ...
        <div className="evaluations-actions-button">
          ...
          <button className="questions-admin-container button">
            <Link to="/notas_desempeno">Evaluacion Desempeño</Link>
          </button>
        </div>
      </div>

      <div className="questions-container">
        <div className="evaluation-h4">Empleados</div>
        {evaluacion.length > 0 ? (
          <div>
            {evaluacion.map((question, index) => (
              <div className="evaluations-result-container" key={index}>
                <p>{question.Nombre}</p>
              </div>
            ))}
          </div>
            ...
            <div>
              <select
                onChange={(e) => {
                  ...
                  setNombreHabilidad(
                    habilidades[selectedIndex].nombre_competencia
                  );
                }}
              >
                <option selected disabled hidden>
              ...
              <button type="button" onClick={() => handleAgregarHabilidad()}>
                Agregar
              </button>
            </div>
