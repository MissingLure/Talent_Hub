...
async function obtener_departamentos(request, response) {
  const errorMessages = [];
  const departamentos = obtenerDepartamentos().then((departamentos) =>
    response.send({ data: departamentos })
  );
}
async function obtener_puestos(request, response) {
  const errorMessages = [];
  const puestos = obtenerPuestos().then((puestos) =>
    response.send({ data: puestos })
  );
}

async function obtenerCompetencia_Habilidad(request, response) {
  try {
    const competenciaHabilidad = await obtenerCompetenciaHabilidad();

    response.json(competenciaHabilidad);
  } catch (error) {
    console.error("Error:", error);
    response.status(500).json({ success: false, message: "Error Server" });
  }
}
...
async function obtener_empleado(request, response) {
  try {
    ...
    response
      .status(500)
      .send({ success: false, details: "Error al obtener el empleado." });
  }
}
...
async function obtener_competencias(request, response) {
  try {
    ...
    response
      .status(500)
      .send({ success: false, details: "Error al tratar de obtener datos." });
  }
}
...
async function obtener_comportamientos_por_competencia(request, response) {
  try {
    ...
    const comportamientos = await obtenerComportamientosPorCompetencia(
      idCompetencia
    );
...
async function get_boss_by_employee(request, response) {
  try {
    ...
    if (boss) {
      ...
      response.send({
        success: false,
        details: "No se le ha asignado un jefe al empleado.",
      });
    }
  } catch (error) {
